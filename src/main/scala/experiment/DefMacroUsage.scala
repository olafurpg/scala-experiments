...
object DefMacroUsage {
  def apply(): Unit = {
    ...
    val results: mutable.Seq[(Path, Seq[MacroCall])] =
      SemanticAnalysis.run(root) { ctx =>
        ...
        ctx.tree.collect {
          case n: Name
              if ctx
                .symbol(n.pos)
                .exists(x => ctx.denotation(x).exists(_.isMacro)) =>
    ...
    Files.write(Paths.get("target", "macros.txt"), s.getBytes)
  }
}
